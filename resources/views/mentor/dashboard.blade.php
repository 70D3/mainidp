<x-mentor.layout title="Dashboard Mentor" :user="$user" :notifications="$notifications">
    ...
    @if ($totalPending > 0)
        ...
        <div
            class="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between shadow-sm mb-6">
            <div class="flex items-center gap-4 mb-3 sm:mb-0">
                ...
                <p class="text-amber-900 font-medium text-sm">
                    Ada <span class="font-bold text-amber-600">{{ $totalPending }} Permintaan</span> yang menunggu
                    validasi anda
                </p>
            </div>
            <a href="{{ $firstPendingMentee
                ? route('mentor.validasi', [
                    'focus_talent_id' => $firstPendingMentee['id'],
                    'tab' => ltrim($firstPendingMentee['pending_tab'] ?: '#exposure', '#'),
                ])
                : route('mentor.validasi') }}"
    ...
    {{-- Daftar Talent: Header di luar card (sesuai pola Finance) --}}
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 mt-2">
        <div class="section-subtitle !mt-0 !mb-0">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path
                            ...
                            <tr class="mentee-row-item hover:bg-gray-50/60 transition-colors"
                                ...
                                <td class="px-5 py-4" style="text-align:center; vertical-align:middle;">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        <div style="text-align: left;">
                                            <p class="font-bold text-base text-slate-800 leading-tight">
                                                {{ $mentee['name'] }}</p>
                                ...
                                <td class="px-5 py-4" style="text-align:center;vertical-align:middle;">
                                    @if ($mentee['status']['pending'] > 0)
                                        <span
                                            class="inline-flex items-center justify-center bg-amber-50 border border-amber-200 text-amber-600 font-bold text-sm px-3 py-1 rounded-full min-w-[36px]">
                                            {{ $mentee['status']['pending'] }}
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center justify-center bg-gray-50 border border-gray-200 text-gray-400 font-bold text-sm px-3 py-1 rounded-full min-w-[36px]">
                                            0
                                        </span>
                                    @endif
                                </td>
                                        ...
                                        @foreach ([['key' => 'exposure', 'stroke' => '#64748b', 'track' => 'rgba(100,116,139,0.15)', 'label' => 'Exposure'], ['key' => 'mentoring', 'stroke' => '#eab308', 'track' => 'rgba(234,179,8,0.15)', 'label' => 'Mentoring'], ['key' => 'learning', 'stroke' => '#22c55e', 'track' => 'rgba(34,197,94,0.15)', 'label' => 'Learning']] as $prog)
                                            ...
                                            <div class="flex flex-col items-center gap-2">
                                                <div
                                                    ...
                                                    <div
                                                        class="flex flex-col items-center font-bold text-[#0f172a] leading-tight">
                                                        <span
                                                            class="text-[17px] tracking-tight">{{ $pct }}%</span>
                                                        <span
                                                            class="text-xs text-[#475569] tracking-tight">{{ $cnt }}/{{ $tgt }}</span>
                                                    </div>
                                                </div>
                                                <span
                                                    class="border border-[#cbd5e1] text-[#475569] bg-white text-xs font-medium px-3 py-1 rounded-full shadow-sm">{{ $prog['label'] }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                </td>
        ...
        @else
            <div class="flex flex-col items-center justify-center py-10 text-center">
                <div class="w-16 h-16 rounded-full flex items-center justify-center mb-4"
                    style="background:linear-gradient(135deg,#c5f0e7,#99f6e4)">
                ...
                <p class="text-base font-bold text-gray-700">Belum Ada Talent</p>
                <p class="text-sm text-gray-500 mt-1 max-w-sm leading-relaxed">Belum ada talent yang terdaftar di bawah
                    bimbingan Anda saat ini.</p>
            </div>
        @endif
    </div>
    </div>
